...
// private log messages are not logged unless they have been explicitly enabled by calling EMFSetLogLevel().
...
static bool AdjustAndVerifyLogLevel(int* level)
{
    ...
    if (EMF_LOG_PRIVATE_DATA & localLevel)
    {
        ...
        localLevel &= ~EMF_LOG_PRIVATE_DATA;
    }
...
void EMFLogBuffer(int level, const char* tag, const uint8_t* buffer, size_t bufferSize)
{
    ...
    for (i = 0; i < bufferSize; i++)
    {
        ...
        if (((i + 1) % 16) == 0)
        {
            EMFLogv(level, tag, "%s", lineBuffer);
            memset(lineBuffer, 0, sizeof lineBuffer);
            lineIndex = 0;
        }
    }
