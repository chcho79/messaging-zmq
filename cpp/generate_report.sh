...
EMF="${PWD}"
EMF_TARGET_OS="linux"
EMF_TARGET_ARCH="$(uname -m)"
USE_TIMESTAMP="yes"
UNITTEST_XML_REPORT="yes"

case $EMF_TARGET_ARCH in
i*86)
EMF_TARGET_ARCH=x86
;;
esac
...
usage() {
    ...
    echo "      -c                                           :  Clean EMF Repository; Should be used to clean existing repository"
...
clean_emf() {
    echo -e "Cleaning ${BLUE}${EMF}${NO_COLOUR}"
    echo -e "Deleting  ${RED}${EMF}/out/${EMF_TARGET_OS}/${EMF_TARGET_ARCH}/debug${NO_COLOUR}"
    echo -e "Deleting  ${RED}${EMF}/.sconsign.dblite${NO_COLOUR}"
    rm -r "${EMF}/out/${EMF_TARGET_OS}/${EMF_TARGET_ARCH}/debug" "${EMF}/.sconsign.dblite"
    find "${EMF}" -name "*.memcheck" -delete -o -name "*.gcno" -delete -o -name "*.gcda" -delete -o -name "*.os" -delete -o -name "*.o" -delete
    echo -e "Finished Cleaning ${BLUE}${EMF}${NO_COLOUR}"
}
...
process_cmd_args() {
    while [ "$#" -gt 0  ]; do
        case "$1" in
            ...
            --target_arch=*)
                EMF_TARGET_ARCH="${1#*=}";
                if [ "x86" != ${EMF_TARGET_ARCH} -a "x86_64" != ${EMF_TARGET_ARCH} ]; then
                    usage; exit 1;
                fi
                shift 1
                ;;
...
generate_report_EMF()
{
    ...
    LD_LIBRARY_PATH="${EMF}/out/${EMF_TARGET_OS}/${EMF_TARGET_ARCH}/debug"
    ...
    tests_list=(
                "${EMF}/out/${EMF_TARGET_OS}/${EMF_TARGET_ARCH}/debug/unittests/emf_api_test"
                "${EMF}/out/${EMF_TARGET_OS}/${EMF_TARGET_ARCH}/debug/unittests/emf_pub_test"
                "${EMF}/out/${EMF_TARGET_OS}/${EMF_TARGET_ARCH}/debug/unittests/emf_sub_test"
               );
